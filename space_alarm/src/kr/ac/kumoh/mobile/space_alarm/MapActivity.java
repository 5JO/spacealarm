...
////////////////////////////
//restart 시에 기존의 도착지 마커 다 지우고 현재 위치로 이동
...
// 검색으로 선택된 지역 마크표시하는 함수
	private void displaySelectedLocation(double lat, double lng, String title) {
		...
		NGeoPoint selectedLocation;
		selectedLocation = new NGeoPoint(lat,lng);
		double distance;
		...
		distance = selectedLocation.getDistance(mMyGeoPoint, selectedLocation);
		Toast.makeText(getApplicationContext(), "현재 위치에서 도착지 까지 거리 :"+String.valueOf(distance), Toast.LENGTH_SHORT).show();
		NMapPOIdataOverlay poiDataOverlay = mOverlayManager
		...
		poiDataOverlay
				.setOnFloatingItemChangeListener(new OnFloatingItemChangeListener() {
					...
					public void onPointChanged(
						...
						mMapController.setMapCenter(point, 13);
						
						distance = point.getDistance(mMyGeoPoint, point);
						Toast.makeText(getApplicationContext(), "현재 위치에서 도착지 까지 거리 :"+String.valueOf(distance), Toast.LENGTH_SHORT).show();
